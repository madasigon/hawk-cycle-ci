...
| `hawk:valkeyEnabled` | on when the relay is on and no `valkeyUrl` is set (non-dev stacks); off on dev envs | Provision a shared ElastiCache Serverless Valkey cluster and wire it into the API, Middleman and the relay. Required for the relay on non-dev stacks; Middleman HA needs it or `valkeyUrl`. Dev envs read this from their own config only. |
| `hawk:valkeyUrl` | _(unset)_ | Use an external Valkey/Redis for Middleman's cache bus instead of the provisioned one. The API and the relay only use a provisioned cluster. Empty disables it. Dev envs inherit it from staging config. With it set nothing is provisioned automatically, so a non-dev stack that keeps the relay on must also set `valkeyEnabled`. |
| `hawk:relayEnabled` | `true` | Deploy the hawk-relay ECS service that backs `hawk acp` / `hawk attach` (operator attach to a running eval's pod). Set `false` to skip its Fargate task + image build if you never attach. On non-dev stacks its session limit needs Valkey, which is provisioned automatically unless `valkeyEnabled` or an external `valkeyUrl` is set; an explicit `valkeyEnabled: "false"` with the relay on is rejected at `pulumi preview`. |
